...
function InitGL(canvasName) {
    ...
    userData.curVelLoc = gl.getAttribLocation(userData.pointProgram, "curVel");
    userData.mvpPointLoc = gl.getUniformLocation(userData.pointProgram, "mvp");
    
    userData.curPosVBO = gl.createBuffer();
    userData.curVelVBO = gl.createBuffer();

    gl.bindBuffer(gl.ARRAY_BUFFER, userData.curPosVBO);
    gl.bufferData(gl.ARRAY_BUFFER, userData.curPos, gl.DYNAMIC_DRAW);
    
    gl.bindBuffer(gl.ARRAY_BUFFER, userData.curVelVBO);
    gl.bufferData(gl.ARRAY_BUFFER, userData.curVel, gl.DYNAMIC_DRAW);
    ...
    userData.vpMatrix.perspective(30, aspect, 1, 1000);
...
function DrawGL(gl) {
    ...
    gl.bindBuffer(gl.ARRAY_BUFFER, userData.curVelVBO);
    if(userData.simMode === JS_SIM_MODE || !userData.isGLCLshared) {
        gl.bufferSubData(gl.ARRAY_BUFFER, 0, userData.curVel);
    }
    gl.enableVertexAttribArray(userData.curVelLoc);  
    gl.vertexAttribPointer(userData.curVelLoc, POS_ATTRIB_SIZE, gl.FLOAT, false, POS_ATTRIB_SIZE * Float32Array.BYTES_PER_ELEMENT, 0);
